...
本地图只对照**两个公开计划**与一个有明确假设的插件运行：赛事方的普希金街封路公告 → CiviFlux 在 18 Sep OSM 有向路网中的两组**合成起终点**条件路由 → AYNA 独立公告中选出的市中心改线街名。城市包只保留机动车可通行路段；独立自行车道和步行道未纳入，本次按 `bus` 权限路由。它不是赛时车速或实际公交轨迹，也不输出“预测准确率”。
