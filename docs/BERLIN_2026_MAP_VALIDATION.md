...
打开 `web/validation.html`，在同一张地图上看灰色道路背景、橙色插件输出的受影响路由路段、紫色情景输入封路候选、蓝色柏林 VIZ 赛事封路通报、VIZ 实时交通状态。接入赛时快照后，红色显示两次快照之间新标记封闭或平均车速降低至少 30% 的 VIZ 路段。点击路段可查 ID、来源、时刻和车速。图层可单独开关。
