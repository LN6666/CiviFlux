...
- GitHub 已对 `main` 启用保护：`core` 与 `container` 检查通过、分支与基线同步、1 位代码所有者批准、过时批准失效、讨论解决、线性历史；禁止强推或删除。仓库管理员当前可绕过保护以处理维护故障，但正常贡献仍应走主题分支与 PR。新账户无写权限时先 fork；不要假定公开仓库等于有推送权限。
