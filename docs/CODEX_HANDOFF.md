...
**交接锚点：**[PR #1](https://github.com/LN6666/CiviFlux/pull/1) 已以 squash 方式合入 `main`（集成提交 `776bf2c`）。其后旧开发分支继续产生的提交不属于 PR #1，必须从最新 `main` 另开分支/PR 审查。当前工作树的 43/9 验收账本、225 项 Python/先前 10 项浏览器证据、Helsinki 案例边界重跑、历史预检与容器修复以最新提交和 CI 状态为准，不把待审内容当成 `main`。
...
4. 在 Linux CI 复验容器构建、固定基础镜像 digest、原生 SUMO 启动与最小权限；记录实际命令/版本/输出。已验证的 macOS native 安装不能替代这一条。Helsinki 第一外圈仅覆盖本轮 48 个可配对候选目标；其余入口映射/核验完成前不可升级城市案例主张。
