...
当前模型按[用户决定](../execution/USER_DECISIONS.md)使用托管 SimpleJev classifier；生产付费调用未授权。以下验收和命令不要求本地模型或 GPU。
...
### G004 托管 SimpleJev 环境预检
验收：检查固定 HTTPS endpoint、指定 classifier、后端凭据、出网许可、调用预算与客户端契约；免费 demo/生产调用及未公开的服务器版本和校准分开标注。未获生产付费授权时记 `DEFERRED_USER`，不下载或启动本地 Qwen。
必须留下实际test/artifact，不以文字宣称完成。
