...
type LayerName='context_roads'|'bcc_pushkin_candidates'|'synthetic_baseline'|'synthetic_new_detour'|'ayna_named_street_candidates'|'street_name_agreement';
...
type Summary={input_candidate_directed_edges:number;unique_new_detour_directed_edges:number;indirect_exact_street_name_agreement_edges:number;proxy_new_detour_edge_name_overlap_pct:number|null;indirect_exact_street_name_agreement_names:string[];synthetic_od_routes:Route[]};
...
function validate(value:unknown):Bundle{
  ...
  if(candidate.schema_version!=='civiflux-baku-indirect-v1'||candidate.status!=='RETROSPECTIVE_INDIRECT_PLAN_COMPARISON'||candidate.case_id!=='baku-f1-2026-pushkin-indirect')throw Error('巴库对照包版本或类型不匹配');
  if(!Array.isArray(candidate.bbox)||candidate.bbox.length!==4||!candidate.summary)throw Error('缺少范围或对照摘要');
...
function inspect(feature:Feature<LineString>):string{
  const p=feature.properties??{};
  const label:Record<string,string>={bcc_input:'BCC 封路输入候选（未人工审阅）',baseline:'无封路合成基线路由',conditional_new_detour:'计算绕行新增路段',ayna_plan_street:'AYNA 公告街名的 OSM 候选（范围未核对）',street_name_agreement:'计算绕行与 AYNA 公告的街名重合',context:'OSM 道路背景'};
...
map.on('load',()=>{
  ...
  map.on('click',event=>{const features=map.queryRenderedFeatures(event.point,{layers:['bcc','agreement','detour','baseline','ayna']});if(features.length)details.textContent=inspect(features[0] as Feature<LineString>);});
  map.on('mouseenter',['bcc','agreement','detour','baseline','ayna'],()=>map.getCanvas().style.cursor='pointer');
  map.on('mouseleave',['bcc','agreement','detour','baseline','ayna'],()=>map.getCanvas().style.cursor='');
...
$<HTMLInputElement>('#bundle-file').addEventListener('change',async event=>{const file=(event.target as HTMLInputElement).files?.[0];if(!file)return;try{show(validate(JSON.parse(await file.text())));}catch(error){status.textContent=String(error);status.dataset.error='true';}});
