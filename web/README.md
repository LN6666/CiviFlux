...
For large CityPacks, road selection uses map clicks or a name/ID search that renders at most 50 matching options. Attention results remain complete in the API result and ZIP, while the panel renders 100 rows per page and reuses search matches until the result or query changes. The full CityPack GeoJSON and completed result are still transferred to the browser; payload size, map frame time and memory must be profiled on target devices before claiming a city-scale performance budget. The single Helsinki outer-pack browser smoke in `evidence/wp5/helsinki_outer_browser_smoke.json` is a scoped local observation, not an SLA.
