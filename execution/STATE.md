...
当前开发线为 draft [PR #7](https://github.com/LN6666/CiviFlux/pull/7) 的 `codex/directed-mapping-review`。下文按提交记录列出的早期 CI/哈希是**历史检查点**；最新受控代码提交 `b7bf9e7` 已由 [GitHub CI 36189858812](https://github.com/LN6666/CiviFlux/actions/runs/36189858812) 的 `core`、`container` 双检查核对通过，CI 临时合并提交与开发提交 tree 完全一致，[CI 摘要](../evidence/wp7/ci_run_36189858812.json)留存。当前 release manifest 重新绑定受控输入；它仍保留 2 个 `DEFERRED_USER`、1 个 `BLOCKED_EXTERNAL`，`engineering_complete=false`。新数据与结果的范围如下：
