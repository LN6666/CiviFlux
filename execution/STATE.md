...
当前开发线为 draft [PR #7](https://github.com/LN6666/CiviFlux/pull/7) 的 `codex/directed-mapping-review`。下文按提交记录列出的早期 CI/哈希是**历史检查点**；最新受控代码提交 `8c02295` 已由 [GitHub CI 36192737662](https://github.com/LN6666/CiviFlux/actions/runs/36192737662) 的 `core`、`container` 双检查核对通过，CI 临时合并提交与开发提交 tree 完全一致，[CI 摘要](../evidence/wp7/ci_run_36192737662.json)留存。当前 release manifest 重新绑定受控输入；它仍保留 2 个 `DEFERRED_USER`、1 个 `BLOCKED_EXTERNAL`，`engineering_complete=false`。新数据与结果的范围如下：
...
- 25 Sep 21:03 UTC 又采集一份官方 VIZ 实时 feed，与 18:45 UTC 原始快照构成**新增限制前的真实双时点安慰剂**。在 375 个可评分 VIZ 有向路段中，背景变化与冻结预测重合 1、邻近变化未重合 13、预测重合处无大变化 10，另 45 不可评分；11 个有效赛前选定对照组的相对降速差中位数为 0.0 个百分点。[聚合回执](../evidence/events/berlin-2026-pre-onset-placebo.json)仅发布时间、哈希和统计；原始数据及[本机安慰剂 GIS 地图](../docs/BERLIN_2026_MAP_VALIDATION.md)留 ignored 路径。已确认浏览器渲染将这些数明确标成赛前背景，**不显示赛事命中率**。这些时段仍有既有马拉松准备，不能解释为纯无事件对照。最新 CI 36189858812 为契约 7、fast 211、data 70 PASS/1 SKIP、outcomes 7、SUMO 11、安全 20、浏览器 16 PASS，容器 PASS；事件小时比较仍待 26 Sep 数据。
- 25 Sep 18:45 UTC 的 6 条 VIZ 马拉松通报含点＋线几何；本机审阅图现提取真实线几何，与公告→OSM 候选有向边比较。[聚合审计](../evidence/events/berlin-2026-viz-mapping-audit.json)显示已开始的 Straße des 17. Juni 候选 **29/37** 条获同街名、足够重合的公告线几何支持，**8/37** 条待复核；Unter den Linden 的 **35** 条候选在此快照无同街名 VIZ 通报。全部 **72** 条仍为未审阅候选，方向核验为 0，公告也不是现场执行。逐段详情和带紫／红／灰输入候选分色的[本机 GIS 图](../docs/BERLIN_2026_MAP_VALIDATION.md)仅存 ignored 路径；橙色插件预测输出保持独立。这一间接核查不是赛时扰动命中率。最新 CI 36191842277 为契约 7、fast 211、data 73 PASS/1 SKIP、outcomes 7、SUMO 11、安全 20、浏览器 17 PASS，容器 PASS；[回执](../evidence/wp7/ci_run_36191842277.json)记录确切树匹配。事件小时比较仍待 26 Sep 数据。
- [柏林输入映射敏感性](../evidence/events/berlin-2026-mapping-sensitivity.json)从原始赛前 VIZ 通报复算 29 条有几何支持的已开始限制边，并以相同城市图/冻结三组 OD 对比原 37 条候选输入；两臂共 6 个路由对象逐项相同。21:03 UTC 第二份通报快照复算仍为 29/8 且结果相同。它只说明移除这 8 条**当前未支持边**没有改变固定三组路由，不能推断 35 条未来候选正确、方向已核验或真实扰动已验证；冻结探针文件未被修改。
- 该映射敏感性代码提交 `8c02295` 的 [Linux CI 36192737662](https://github.com/LN6666/CiviFlux/actions/runs/36192737662) 已通过：契约 7、fast 211、data 73 PASS/1 SKIP、outcomes 7、SUMO 11、安全 20、浏览器 17 PASS，容器 PASS；[回执](../evidence/wp7/ci_run_36192737662.json)核对确切树、日志与产物哈希。CI 缺少 ignored Helsinki 城市包的那一项仍跳过，不能把映射审计写成赛时命中率。
