...
当前开发线为 draft [PR #7](https://github.com/LN6666/CiviFlux/pull/7) 的 `codex/directed-mapping-review`。下文按提交记录列出的早期 CI/哈希是**历史检查点**；新增 Berlin/Baku 城市图、间接 GIS 对照及多方式候选网络，最新受控代码提交 `2eec054` 已由 [GitHub CI 36184549496](https://github.com/LN6666/CiviFlux/actions/runs/36184549496) 的 `core`、`container` 双检查核对通过，CI 临时合并提交与开发提交 tree 完全一致，[CI 摘要](../evidence/wp7/ci_run_36184549496.json)留存。当前 release manifest 重新绑定受控输入；它仍保留 2 个 `DEFERRED_USER`、1 个 `BLOCKED_EXTERNAL`，`engineering_complete=false`。新数据与结果的范围如下：
...
- 本机完整非 live 产品测试 246 PASS/1 SKIP，新增 Berlin 比较器反例测试和 Web build 通过。本机 Playwright 无 Chromium 可执行文件；相同 `2eec054` tree 的 Linux CI 补齐了浏览器运行：契约 7、fast 211、data 63 PASS/1 SKIP、outcomes 7、SUMO 11、安全 20、浏览器 15 PASS、容器真实 API/合成 SUMO smoke PASS。data 的 1 项跳过因 CI 与本开发工作树未带 ignored Helsinki 城市包，不能视为发布所需真实数据验证通过。城市原始 PBF、VIZ 快照与大型 CityPack/KG 保持 ignored，本仓库只提交脚本、小型来源卡和哈希证据。
