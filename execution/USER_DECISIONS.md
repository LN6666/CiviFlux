...
## 道路级 GIS 实测对照 — 2026-09-25

- 用户明确要求按开源产品验收方式核对**插件判断受影响地段与真实交通数据之间的差距**，优先给出 GIS 地图可视化、逐段命中/漏报/误报与具体数据来源，而不是以论文式边界声明代替实测。
- Berlin 2026 的冻结插件路由输出和情景封路输入须分开显示；官方实时交通、官方通报也须分开。计划通报不能冒充现场观测，赛事增量前的快照不能冒充赛时结果。
- 从柏林官方 VIZ WFS 采集带时间戳的交通图层，原始 VIZ/HERE 快照仅保留本机 ignored 路径。赛前/赛时快照按 VIZ 路段 ID 配对，并在 GIS 地图展示具体车速、封闭状态及空间差集；无需 Featherless 订阅。
