...
- 当前受控代码提交 `620bc30` 的 [Linux CI](https://github.com/LN6666/CiviFlux/actions/runs/35921499626) 中契约 7、fast 210、outcomes 7、SUMO 11、安全 20、浏览器 11 项通过；CI 数据组 49 PASS/1 skip（无 ignored 城市包），本机带已有 Helsinki 当前快照为 [50 PASS](../evidence/wp7/current_data_test_620.json)。这些是分组结果，不混称独立科学验证。Linux 无网络只读容器 API/SUMO smoke 通过；本机当前离线 clean checkout 因 UV 缓存缺少固定 `sumo-data` wheel 而 `BLOCKED_ENVIRONMENT`，不能沿用旧代码树的本机 PASS。
...
| G101 | 下载并冻结OSM/GTFS | PASS | Helsinki 的 OSM/GTFS 和 Berlin/Baku 的定日 OSM 字节均记录来源、日期、SHA256 与许可；Baku 赛前源不等于实际道路管制。 [citypack_audit.json](../evidence/wp1/citypack_audit.json)、[Berlin KG audit](../evidence/events/berlin-2026-city-kg-audit.json)、[Baku KG audit](../evidence/events/baku-2026-city-kg-audit.json)、[README.md](../data/README.md) |
...
| G301 | 真实typed temporal KG投影 | PASS | 版本化 typed temporal 投影具来源、方向和有效窗；公交静态依赖与 operational 图分开。Berlin/Baku 的当前道路快照还导出 ontology objects、来源/转向/候选设施接入 typed links，留有来源和车种作用域；不把静态 KG 当作 event PPR 或实际封路。 [final_python_junit.xml](../evidence/wp7/final_python_junit.xml)、[ontology_review.md](../evidence/wp0/ontology_review.md)、[Berlin KG audit](../evidence/events/berlin-2026-city-kg-audit.json)、[Baku KG audit](../evidence/events/baku-2026-city-kg-audit.json) |
...
| G602 | 公平运行A0–A5 | DEFERRED_USER | A0/A1/A2/A5 共 192 行共享 48 套物理事实；A4 的无 PPR 一步关系排序核已在固定离线策略下运行，覆盖 23 个设施候选、物理 facts 与 A2 相同；这不是模型实验。A3/A4 共 96 行生产模型实验仍未跑，免费图 smoke 不能替代完整消融。 **待补：**待生产调用获用户配额授权且独立模型评估设计冻结后执行 A3/A4 及完整负对照；保持相同 physical cache。 [ablation_report.json](../evidence/wp6/ablation_report.json)、[a4_direct_control_offline.json](../evidence/wp6/a4_direct_control_offline.json)、[final_gate.json](../evidence/wp6/final_gate.json) |
...
| G701 | clean checkout重建 | PASS | 当前代码提交 `620bc30` 的 [GitHub Linux CI 35921499626](https://github.com/LN6666/CiviFlux/actions/runs/35921499626) 从干净检出构建固定 base 的 amd64 镜像，记录本地镜像内容摘要（未发布 registry 镜像），在无网络只读容器完成真实 API/export 与合成需求 SUMO 1.27.1 smoke，原生 SUMO/netconvert 无缺失库。当前 macOS 原生全离线尝试因本机 UV 缓存缺少固定 `sumo-data` wheel 于依赖安装阶段 `BLOCKED_ENVIRONMENT`；旧提交 `ada0475` 的 warm-cache PASS 不能升级为当前提交的原生 PASS。此项仅在 Linux 容器范围内 PASS，不验证历史城市影响或付费模型。 [ci_run_35921499626.json](../evidence/wp7/ci_run_35921499626.json)、[clean_checkout_620_offline.json](../evidence/wp7/clean_checkout_620_offline.json) |
